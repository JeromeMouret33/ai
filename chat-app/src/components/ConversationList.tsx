...
interface Props {
  ...
  /** Called each time fresh data arrives so the parent can index it. */
...
export default function ConversationList({
  ...
  useEffect(() => {
    void fetch_();
    const id = setInterval(() => void fetch_(true), pollMs);
    ...
    <ul className="divide-y divide-gray-100">
      {conversations.map((conv) => (
        <li key={conv.id}>
          ...
          >
            <Avatar name={conv.name} avatarUrl={conv.avatarUrl} />
